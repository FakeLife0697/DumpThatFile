...
def encrypt_rsa(aes_key: str, public_key: str = None):
    cipher_aes_key = None
    try:
        if public_key:
...
def decrypt_rsa(cipher_aes_key: str, private_key: str = None):
    decrypted_key = None
    try:
        if private_key:
...
def generate_aes_key():
    ...
    try:
        ...
        print("Process failed: AES generation")
        print(e)
        
    return key, iv

def encrypt_aes(file, key: str = None, iv: str = None):
    cipherfile = None
    try:
        ...
        filename = file.split(".", )
        filename[0] = filename[0] + "_test"
        cipher_filename = ".".join(filename) + ".enc"
        
        with open(file, 'rb') as fin, open(cipher_filename, 'wb') as fout:
            fout.write(iv)
        ...
        print("Process failed: AES encryption")
        print(e)
    
    return cipherfile

def decrypt_aes(cipherfile, key: str = None):
    decrypted_file = None
    try:
        key = base64.b64decode(key.encode('utf-8'))
        ...
        print("Process failed: AES decryption")
        print(e)
...
def compare(hash1 = None, hash2 = None):
    return hash1 == hash2

file = "D:/Coding/IT 8/Thesis/DumpThatFile/tests/hidden.sh"
filename = file.split(".", )
filename[0] = filename[0] + "_test"
cipher_filename = ".".join(filename) + ".enc"
...
encrypt_aes(file, key, iv)
hash1 = hash(file)

cipherkey = encrypt_rsa(key, public_key)
decrypted_key = decrypt_rsa(cipherkey, private_key)

decrypted_file = decrypt_aes(cipher_filename, decrypted_key)

hash2 = hash(decrypted_file)

print(hash1)
print(hash2)

print(compare(hash1, hash2))